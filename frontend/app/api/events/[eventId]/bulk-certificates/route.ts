...
/** @deprecated — `/bulk-materials` category=CERTIFICATE ishlating */
export async function POST(
  ...
  try {
    ...
    const rl = await checkRateLimit(`user:${user.id}:bulk-certs`, 10, 60 * 60 * 1000)
    if (!rl.allowed) {
    ...
    console.error('Bulk certificates error:', error)
