...
function sizeOf(template: PreviewTemplate) {
    ...
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 flex max-h-[88vh] w-[95vw] max-w-5xl -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden rounded border border-border bg-surface shadow-lg focus:outline-none md:flex-row">
          <div className="flex min-h-72 w-full shrink-0 items-center justify-center bg-surface-secondary p-5 md:min-h-[36rem] md:w-[56%]">
            <div
              className={cn(
                'relative overflow-hidden bg-white shadow-md ring-1 ring-black/10',
                isLandscape ? 'w-full' : 'h-full'
              )}
              style={{ aspectRatio: `${size.width} / ${size.height}` }}
            >
              <Image src={previewSrc} alt={title} fill className="object-contain" unoptimized />
            </div>
          </div>
